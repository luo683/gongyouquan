...
后端骨架与 `auth / groups / members / messages / sync` 五个垂直切片均已落地，并已在**真实 PostgreSQL 17.11** 上跑通、固化成集成测试；**已读回执、typing、presence（含在线快照）、@提及四条都已两侧打通**；`sync:hello` 已接通，服务端 `contractVersion` 现在真的会到浏览器；浏览器端（React + Vite）可真聊、未读徽标会消、成员管理可逐条点通、能 @ 人并收到「@我」列表。运维闭环起步：`/internal/metrics` 已按「只报能真实测到的」实现；`infra/backup/` 五个脚本 + `Dockerfile` 已在 compose 里接成 `backup` 服务，由 supercronic 调度、`backup-health.sh` 做新鲜度探活；**恢复演练不是文档而是真跑过**——本轮跑的是「从 restic 仓库里取回来的那一份」，七道检查全绿，外加五条各自以 1 退出的负向用例。**`backup-once.sh` 的异地那一段（restic）也是这一轮第一次被执行**：之前 `RESTIC_REPOSITORY` 一直是空的，所以那四行从来没被检验过——跑通之后发现手册 8.2 的 `restic forget` 缺 `--group-by tag`，而 dump 文件名带时间戳使每晚快照各成一组，**保留策略实际上一个都删不掉**（双向实测），连同另外四处偏离一起登记在 `decisions/0014`。部署栈的密钥已改成必填插值，缺任何一个 `docker compose config` 直接拒绝解析（见 `decisions/0012`）；备份容器与手册之间的三处出入登记在 `decisions/0013`。**告警终于有落点了**：`POST /api/v1/hooks/alert` 做 HMAC 验签、幂等键与 5 分钟聚合窗口，命中后把一行告警写进系统群（同事务顺带一条 outbox 索引事件）；`infra/backup/notify-alert.sh` 是它的第一个（也是目前唯一一个）发送方，`backup-once.sh` 那条 `ERR` trap 从此指向一个真实存在的文件，`presence.onDrift` 也不再只打 `console.error`。**签名的取舍全部登记在 `decisions/0015`**，其中第五节是一个没有解决的缺口：7.1 设想的 Uptime Kuma 签不出这个名。**三道闸门本地全绿（239 用例），且 `/hooks/alert` 是真在跑起来的栈上验过的**，不是 dry run。**仍不可对外部署**——缺的是 `tasks / files / search` 三个整块未动的模块（`ops` 已经不是，但它只有 `/hooks/alert` 这一条路，**审批闭环**还没有；outbox 的 writer 侧已符合 6.9，worker 要跟 `search` 一起做，见 `decisions/0016`）、Electron 外壳、systemd / `opsctl`。**数据库、核心收发链路、以及「备份到底能不能恢复」这一条不再是阻塞项**；备份还缺的两件都不是代码：一个不在同一台机器上的 restic 仓库，和一把私钥放在机器外的 age 密钥。
...
pnpm test        无库机器：contracts 31 + web 9 + server 113 通过，server 另有 86 个真库用例整片 skip（8 个文件）
真库闸门         INTEGRATION_DATABASE_URL 设上后 server 199 全过（27 文件，含真库集成与 e2e 双人聊天）
合计             32 文件 / 239 用例，全绿（2026-09-15 用 §9 那份 Node 22 复跑确认）
...
2. **整块未动的模块**：`tasks` / `files` / `search` 三个仍是空的。**`ops` 已经不再是**，但它只落了 `/hooks/alert` 这一条路：手册 7.x 的 `/hooks/ops-report`、`/hooks/ops-request`、`/ops/requests/:rid/decision`、`/ops/requests` 都还没有，**运维请求的审批闭环（谁批、批了之后谁执行）一条都没有**；另外 `alert_windows` / `alert_events` 两张表已经有了、`/hooks/alert` 的幂等与 5 分钟聚合也已经有了。**outbox 这一头不要顺手去做 worker**：writer 侧现在已符合 6.9（payload 自包含、撤回写 `delete`，见 `decisions/0016`），但 worker 的唯一规定终点 Meilisearch 在这个栈里根本不存在，先 drain 只会把「没人消费」变成四个永久绿灯（巡检项 18、`readyz`、`outboxPending`/`outboxLagSeconds`）——它要跟 `search` 模块一起做，不是 `ops` 的一条独立待办。`processed_at` 现在永远为空、`readyz` 的 lag 因此单调上涨，这是**已知且如实**的状态（§11 坑 17 与 `decisions/0006` 缺口六）。
...
| `0016-outbox-payload-self-containment-and-delete-events.md` | outbox 的 **writer 侧违反了 6.9 的两句话**，而 `0001_init.sql` 那行列注释（`自包含索引文档，worker 不回查业务表`）本来就写着：①payload 当时只有 `{messageId, groupId, seq, reason}`，是一个**指针**——补自包含不只防「索引到旧内容」，更因为**「索引范围」要看 `kind`，而 `kind` 只在业务表里**，所以「不回查」与「带够字段」是同一条规则的两半，只补前者等于没补；②撤回三个 reason 全写 `upsert`，而撤回是**软删**（`body` 原地留着给 `/raw`），于是**两处的修法有顺序依赖**：只补 payload，一个自带被撤回正文的 `upsert` 事件会把它原样送进索引且再没有后续事件纠正——`delete` 是 payload 自包含的前提，`delete` 的 payload 刻意不带 `body`（不给被撤回的正文多一个副本和多余一份保留期）；③**索引范围活在哪一层**说明书没说，默认实现选「writer 全量写、worker 按 `kind` 裁」，代价要如实写：巡检项 18 与 `/readyz` 量的是「未消费」不是「未索引」，这两个词在说明书里是同一件事，在这个实现下不是（今天会被裁掉的只有 `kind='system'`，即运维告警那一行——**它进不进搜索索引是产品问题**）；④**worker 不是下一件事**：三段结构的第三段（Meilisearch）在这个栈里不存在，先 drain 会把「没人消费」换成四个永久绿灯，今天那条单调上涨的 lag 至少是真话——降级路径的数据库那一半（`pg_trgm` + `messages_body_trgm`）其实随迁移 0001 就落了；⑤没有迁移（改已应用的迁移会被 checksum 挡），存量行按「`reason='send'` 且从未编辑未撤回 → 形状回填，否则补发新事件、别改旧行」处理，本地那 4 行演练遗留已回填 |
...
- `outbox.payload`：**带正文**（`body`/`kind`/`senderId`/`createdAt`），worker 侧不许回查 `messages`。看起来像「同一份数据存两处、应该规范化掉」，但那正是 6.9 立这条的理由：回查答的是「现在是什么」不是「这条事件当时说了什么」，见 `0016` 一节。
- `outbox`：**撤回写 `event_type='delete'`，不是第三个 `upsert`**。`delete` 的 payload 只有文档键、没有正文。想把 revoke 也走 upsert 的直觉会连着上一条把被撤回的正文原样送进索引（撤回是软删，`body` 还在 `messages` 里），而且再没有后续事件来纠正它。
...
9. 之后才往 `tasks / files / search` 走——这是仅剩的三个整块未动的模块。`ops` 已经有了第一条路（`/hooks/alert` + `notify-alert.sh` + `presence.onDrift`），下一件是**那条审批闭环**：`/hooks/ops-report`、`/hooks/ops-request` 与 `/ops/requests/:rid/decision`——它比路由更重，因为「批了之后谁执行」在说明书里也是没定的。**outbox worker 不在队列里**：writer 侧已符合 6.9（`decisions/0016`），而 worker 必须跟 `search` 模块一起做（Meilisearch 进 compose + 查询接口 + 降级路径 + worker 四件是同一件事的四半，拆开做会留下假绿灯）。`readyz` 的 `outboxLag` 会一直涨，那是**已知且如实**的状态（§11 坑 17、`decisions/0006` 缺口六），不是等着被消的债。
